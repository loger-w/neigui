...
describe("BrokerAggBarSvg", () => {
    it("renders bars matching InstBarSvg shape", () => {
        const { container } = render(_jsx(BrokerAggBarSvg, { data: [10, -20, 30], width: 300, height: 50, label: "\u5206\u9EDE (1)" }));
        const rects = container.querySelectorAll("rect");
        expect(rects.length).toBeGreaterThan(0);
    });
    it("renders selected-day cursor at correct X", () => {
        const { container } = render(_jsx(BrokerAggBarSvg, { data: [10, 20, 30, 40], width: 400, height: 50, label: "", selectedIndex: 2 }));
        expect(container.querySelector("[data-testid=sel-cursor]")).toBeTruthy();
    });
    it("does not render cursor when selectedIndex is null", () => {
        const { container } = render(_jsx(BrokerAggBarSvg, { data: [10, 20, 30], width: 300, height: 50, label: "", selectedIndex: null }));
        expect(container.querySelector("[data-testid=sel-cursor]")).toBeFalsy();
    });
});
describe("InstBarSvg selectedIndex", () => {
    it("renders selected-day cursor at correct X", () => {
        const { container } = render(_jsx(InstBarSvg, { data: [10, 20, 30, 40], width: 400, height: 50, selectedIndex: 2 }));
        expect(container.querySelector("[data-testid=sel-cursor]")).toBeTruthy();
    });
    it("does not render cursor when selectedIndex is null", () => {
        const { container } = render(_jsx(InstBarSvg, { data: [10, 20, 30], width: 400, height: 50, selectedIndex: null }));
        expect(container.querySelector("[data-testid=sel-cursor]")).toBeFalsy();
    });
});
describe("MarginLineSvg selectedIndex", () => {
    it("renders selected-day cursor at correct X", () => {
        const { container } = render(_jsx(MarginLineSvg, { marginData: [10, 20, 30], shortData: [5, 15, 25], width: 400, height: 50, selectedIndex: 1 }));
        expect(container.querySelector("[data-testid=sel-cursor]")).toBeTruthy();
    });
});
describe("KlineChartSvg click + selectedIndex", () => {
    ...
    it("renders selected-day cursor with date tag", () => {
        const { container, getByText } = render(_jsx(KlineChartSvg, { candles: candles, width: 500, height: 300, selectedIndex: 3 }));
        expect(container.querySelector("[data-testid=sel-cursor]")).toBeTruthy();
        expect(getByText("2026-06-13")).toBeTruthy();
    });
    it("does not render cursor when selectedIndex is null", () => {
        const { container } = render(_jsx(KlineChartSvg, { candles: candles, width: 500, height: 300, selectedIndex: null }));
        expect(container.querySelector("[data-testid=sel-cursor]")).toBeFalsy();
    });
    it("fires onClickIndex with correct index", () => {
        const onClick = vi.fn();
        const { container } = render(_jsx(KlineChartSvg, { candles: candles, width: 500, height: 300, onClickIndex: onClick }));
        const overlay = container.querySelector("rect[data-testid=overlay]");
