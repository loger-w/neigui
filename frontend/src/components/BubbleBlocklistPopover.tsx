            ...
            candidates.map((c) => (
              ...
              >
                <span
                  ...
                  title={formatBrokerName(c.id, c.name)}
                >
                  {formatBrokerName(c.id, c.name)}
                </span>
      ...
      {N === 0 ? (
        ...
        blocked.map((b) => (
          ...
          >
            <span
              className="flex-1 min-w-0 truncate text-ink-muted"
              title={formatBrokerName(b.id, b.name)}
            >
              {formatBrokerName(b.id, b.name)}
            </span>
            <button
              type="button"
              data-testid="bubble-blocklist-remove"
              onClick={() => onRemove(b.id)}
              aria-label={`移除 ${formatBrokerName(b.id, b.name)}`}
