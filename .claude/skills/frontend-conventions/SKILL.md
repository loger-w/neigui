...
- **日期值直接進查詢時一律用 `DateField` 的 `onValueChange`**(2026-09-25 fix/date-field-partial-input 翻新):原生 date input 清除 / 逐位輸入年份會送出空值與 `0002-`/`0020-`/`0202-` 中間值,直接接 queryKey = 每鍵一輪請求;`snapToTradingDay` 還會把早於清單的半成品夾到最早交易日(個股頁曾跳到一年前)。DateField 包裝路徑(有 `snapToDates` 或 `onValueChange`)已內建防護:只放行完整 `YYYY-MM-DD` 且在 [`min` ?? 2000-01-01, `max`],其餘存內部草稿(欄位照顯示輸入)、blur 或外部 `value` 變動時丟棄。**不要用 `onChange` 讀原始值去設查詢 state**(純原生路徑無防護;選擇權頁曾因此打出半成品請求)。`BrokerFlowsPanel` 另在純原生路徑上自帶一套草稿防護(多 300ms debounce + render 時現算「今天」上限),屬既有重複,見 next-time。Trigger:新增日期欄位、或把日期欄位接到查詢時。
- **原生控制項的深色與可控外觀邊界**(2026-09-24 mod/date-field-dark-calendar → 2026-09-25 mod/root-color-scheme-dark 修正,Chromium 149 實測):月曆彈窗、number 上下鈕、預設焦點框由瀏覽器依 `color-scheme` 繪製(`<select>` 展開清單底色沿用 select 自身 bg,只有選中列跟 scheme:light 藍底白字 → dark 淺藍底深灰字,對比 5.4→6.4:1)→ 整站只有深色,**在 `:root` 宣告 `color-scheme: dark` + index.html `<meta name="color-scheme" content="dark">`**(繼承屬性,元件不必各自宣告;Chromium 捲軸不受影響,因 `*` 已明確指定 `scrollbar-color`;Safari 不支援該屬性,捲軸會隨 dark 轉深 — 未實測。曾誤以「會改捲軸」為由只掛在 DateField,已被 review 推翻)。root 轉 dark 的連帶可見變化:**瀏覽器預設鍵盤焦點框**(沒自訂焦點樣式的按鈕)由黑外圈為主轉白外圈為主(深色頁面上更明顯);一般文字反白不變。dark 下月曆圖示改畫淺色,`::-webkit-calendar-picker-indicator` 的 `invert()` 要反算(0.65→0.35 收合外觀逐像素不變);Firefox / Safari 不支援該 pseudo-element,圖示為原生 — 依規格推論隨 dark 轉淺色,未實測。**鍵盤編輯中的選取段反白、彈窗內選取格作者 CSS 改不動**(pseudo `:focus` / `!important` / `accent-color` / `::selection` 皆無效)— 要全面吃專案配色只能自製元件。驗這類 CSS 用 Playwright 截圖 + 頁內 canvas 取像素,先算「疊色理論值」再比,並排除同畫面其他同色系元素(本案曾把 focus ring 的 accent/40 誤認成反白)。Trigger:改日期欄位外觀、新增原生 `<select>` / number / checkbox 等控制項、改 `:root` 的 `color-scheme`、或 user 要求原生控制項吃專案配色時。
- **搜尋下拉「失焦延遲關閉」必須在回到輸入框時取消計時器**(2026-09-25 fix/e34-symbolsearch-option-flake):`onBlur` 排 `setTimeout(() => setOpen(false), 150)`(讓 option 的 mousedown 先跑)時,`onFocus` 與 `onChange` 都要 `clearTimeout` — 否則 150ms 內點回來打字,剛開的下拉會被舊計時器關掉;主執行緒忙(剛選股在畫圖)時下拉連一瞬都等不到,e2e 表現為 `getByRole("option")` 0 筆的 flake(E34 修前 7/30 紅)。計時器 id 必存 ref(`onBlur={() => setTimeout(...)}` 無法取消)。樣板 `SymbolSearch.tsx::cancelPendingClose` + 假時鐘 vitest(失焦 → 10ms 內 focus / 直接輸入 → 過 200ms 仍開;沒回來照常關)。同病未修:BrokerSearch / BorrowFeeStockFilter / BrokerFlowsPanel(見 next-time)。Trigger:新增或修改任何 combobox / 搜尋下拉的開關邏輯時。
