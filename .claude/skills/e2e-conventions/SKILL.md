...
- **rotate 必連動重跑 `scripts/gen-market-e2e-fixtures.py`**(2026-07-20 populated market fixture):market EOD 窗口 fixture(TaiwanStockPrice universe / TAIEX)的日期範圍 = FAKE_TODAY − 198 天,寫死在檔名與 rows;換基準日改腳本 `END` 重生 + 刪舊窗口檔,否則四個 EOD 欄位靜默退回 null(M9 會紅)。手算基準(McClellan 10.0 / vol ratio 2.50 / share 60.0%)與 M9 assertion 同源,改腳本常數要同步 spec。
