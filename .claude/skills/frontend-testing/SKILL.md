...
- **jsdom 下 bubble-svg 泡泡點擊可用「circle cx/cy 直擊」測真實 click 路徑**(2026-07-27 feat/bubble-multi-broker):svg 的 getBoundingClientRect 在 jsdom 全零 → `chip-bubble-svg.tsx::hitTest` 的 `mx = clientX - rect.left` 退化為 `mx = clientX`,對 main overlay `fireEvent.click(overlay, { clientX: +circle.getAttribute("cx"), clientY: +cy })` 即可命中該泡泡,免 mock BubbleChartSvg(`ChipBubbleView.test.tsx`「泡泡 / 明細列入口 toggle」是樣板)。Trigger:要測泡泡點擊 / hitTest 相關行為時。
