...
- **TWSE RWD**(`www.twse.com.tw/rwd/zh/...`):官網內部 JSON API,**無 swagger**、可帶 date 參數可回溯;`stat != "OK"` = 無資料(不是錯誤,回空即可)。月批次端點(BFIF8U)date 給任一天回整月。
  ...
  - **除權息計算結果表 TWT49U**(2026-07-14 現增 event-study 實測):`exRight/TWT49U?startDate=YYYYMMDD&endDate=YYYYMMDD` 可回溯多年、範圍可跨月;「權/息」欄分權/息/權息,但**認購價在 detail**:`exRight/TWT49UDetail?STK_NO=<代號>&T1=<YYYYMMDD>`(參數即列表「詳細資料」欄的 `"4989,20260629"` 兩段;**detail 成功 stat 是小寫 "ok"**,錯參數回 `{"stat":"系統忙碌中…"}` 非 404)。含現金增資配股率/認購價,錨點驗證:4989@20260629 → 認購價 60、總配股率 ≈0.254。間歇 307 會發生,重試即可。
- **TPEx OpenAPI**(`www.tpex.org.tw/openapi/v1/...`):有 swagger(`/openapi/swagger.json`,225 paths);**一律只回當月/最新,無歷史參數** — 歷史資料 cache 即唯一副本(見 cache 節)。
  - **但 TPEx 舊站 php 有歷史**(2026-07-11 warrant-iv-drift 實測翻新「TPEx 歷史不可得」認知):`.../web/stock/aftertrading/otc_quotes_no1430/stk_wn1430_result.php?l=zh-tw&d=<民國Y/MM/DD>&se=WW` 回溯 ≥3 年(權證行情 17 欄含最後買/賣價)。**`se` 枚舉是致命參數**(2026-07-17 /bug tpex-warrant-iv-empty 翻新):`WW` = 權證表(實測 9,075 筆全 7xxxxx,== issue universe)、`EW` = 上櫃股票+ETF「**不含**權證、牛熊證」、`AL` = 全部 — 曾誤用 `EW` 導致 TPEx 權證線靜默全滅 63 日檔;probe 驗表**必看回傳代號分佈**(EW 回 1,013 筆股票也是 http 200 / stat ok,「有 rows」≠「是對的表」)。shape = `{date: 西元緊湊 echo, stat: 小寫 "ok", tables: [{fields, data}]}`;**欄序由 fields stripped 名稱對照解,勿硬編 index**(舊年份量欄名「張數→千股」變體);**取表前必四欄名齊備 guard** — 缺欄 `fields.index()` 的 ValueError 非 httpx 例外,會穿透只 catch httpx 的逐日迴圈炸整段(code-review CR-A1 實證);echo date 不符指定日視為空;回傳代號仍一律過條款 universe 交集(防禦)。樣板 `warrant_iv_history.py::parse_wn1430`。
  - **上櫃除權息計算結果(含現增認購價)也在舊站**(2026-07-14 實測):`/web/stock/exright/dailyquo/exDailyQ_result.php?l=zh-tw&d=<民國起日 YYY/MM/DD>&ed=<民國迄日>`,**必須 d+ed 成對給**(只給 d 或用 sd/ed 會被靜默忽略而回最新資料 — 以回傳 rows 的除權息日期落在請求範圍內做 guard)。範圍可跨整年;21 欄含現金增資股數/認購價/原股東認購股數/按持股比例仟股認購(/1000 = 每股認股率)/每仟股無償配股。新站 `/www/zh-tw/exRight/*` 猜測路徑皆 404,preAnnounce 只回最新。
