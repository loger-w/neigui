...
## Phase 5 量測結果(2026-07-15 17:43-17:52,方法與 Phase 1 完全相同)

| 情境 | Before | After | 目標 | 判定 |
|---|---|---|---|---|
| 冷:今日快照未 build,使用者開 tab(DevTools) | ≈46s | **0.6s**(warrants 68ms / quotes 604ms;預熱已接手) | ≤5s | ✓ −98.7% |
| 冷最壞:backend 啟動 0 秒即請求,TWSE cache 冷(bench) | ≈46s | 5.6s(join 預熱殘餘) | ≤5s | ⚠ 邊緣 — TWSE 端未命中重算 10-20s 不可壓縮;正常使用(啟動後 >20s 才點)碰不到 |
| 熱:backend mem 熱(DevTools) | 0.35s | **0.27s**(warrants 21ms / quotes 274ms) | ≤2s | ✓ |
| 熱:盤中 MIS 慢批(baseline 1.78s 貼線) | 1.78s | S4 並發 ≤3,理論 ~0.7s(盤後無法重現盤中批延遲;行為由測試鎖住) | ≤2s | ✓(margin 擴大) |

**沒退化的其他 metric**:chip endpoints 同輪量測 419-630ms(與 baseline 同量級);backend 710 tests 綠;console 0 errors/warnings;記憶體 — snapshot mem 常駐與優化前相同(原 lazy 路徑 build 後同樣常駐)。新增成本 = 每次啟動 + 每日凌晨各一次背景 build(~10 個 TWSE/TPEx 請求,零 FinMind 配額)。

驗證截圖:`docs/specs/perf-warrant-api-load/screenshots/warrant-tab-warm-open.png`
