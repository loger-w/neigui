...
## Root bottleneck(Phase 2 實驗定案)

1. **[主因] iv backfill 啟動即轟 TWSE,與冷 build 撞同 host**:E3 對照 — 停 backfill 後冷 build 43.5s → **4.62s**。backfill 每次啟動對永遠缺檔的週末日(60 日窗 ~34 個)發 4 個 MI_INDEX + 5s retry sleep,TWSE 慢查詢(歷史現算 10-25s)期間同 host 請求全面排隊。
2. **[次因] `_build_snapshot` 六個 upstream fetch 全序列**:E3 分解 MI×2(1.0s)+ t187(0.06s)+ TPEx×3(2.4s)序列 ≈ 4.1s;E1b 證明並發無礙。
3. IV 反解 37,957 檔僅 0.5s — 舊線索「63s = IV 反解」證偽,lifespan 預熱 / thread offload 的「IV 成本」動機不存在,但預熱仍是消滅使用者可見冷路徑的正解。
