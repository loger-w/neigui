...
### P4 sector amount share 慣例(2026-07-02 /feat market-sector-amount-share 沉澱)

- **Extract 函式餵除法 aggregation 前要 clamp domain-impossible 值**:P4 `_extract_amount_by_stock` 若直通負 Trading_money,下游「sector_today>0 ⟹ today_total>0」除法不變量被 corrupt data 打破 → ZeroDivisionError 穿透 httpx-only catch → 整 `/api/market/snapshot` 500。負 turnover 視同 corrupt → 0.0(對齊「非數值 → 0.0」慣例)。Trigger:寫任何新 extract/parse 函式,其產出會進入除法或比值 aggregation 時。
- **Phase 4 review 產出的 lock test 走 mutation 抽驗,不硬湊 red-first**:test-gap finding(recency 未鎖 / except 寬度未鎖)鎖的是「已正確行為」,天生無紅可寫。證據改用 mutation:手動改壞(`reverse=True→False`、`except httpx→Exception`)→ 跑 lock test 見紅 → `git checkout` 還原 → 見綠。commit 訊息註 `mutation-verified`。Trigger:Phase 4 review 後補 lock test 時。
- **Adversarial verify split-vote 要拆 fix 成分裁決**:P4 CORR-1 一票 confirm(executable repro 成立)一票 refute(fix b 違反 KG7 自然解設計決策)— 兩票都對,對象不同。處置:接受 fix a(extractor clamp)、拒絕 fix b(aggregate 加特殊分支),不整條 accept/reject。Trigger:multi-lens review 的 finding 出現 1:1 split vote 時,先檢查兩票是否在講 fix 的不同成分。
- **共用 cache_key 含日期 ⟹ 每日首請求必冷**:`breadth_prices_<start>_<end>` 的 end = 當日 → 日期翻頁後 key 變 → 當日第一個 snapshot request 付冷啟動(fetch loop ~240s;增量 fetch 留 next-time)。**別把當日首請求的 4 分鐘當 regression 修**。2026-07-02 /perf 後其餘數字已翻新:warm <1s(`eod_results_*` result cache)/ refresh 不進 EOD / 冷啟動期間其他 endpoint 不再被卡 — 舊值 warm 36.8s、refresh 278s 不再適用。Trigger:real-env 驗證 snapshot 端點覺得「怎麼突然變慢」時。
- **:8000 被占先查 CommandLine 再決定殺**:`(Get-CimInstance Win32_Process -Filter "ProcessId = <pid>").CommandLine` 若帶 `--reload` = dev server 已 hot-reload 新 code,直接拿來 real-env 驗證,不用殺(§9 既有「zombie 殺進程」條目只適用非 --reload 殘留)。Trigger:Phase 6 real-env 撞 port 被占時。

### Snapshot hot-path 慣例(2026-07-02 /perf snapshot-hot-path 沉澱)

- **`json.load`/`json.dump` 是單一 C call,整份文件 parse/serialize 持 GIL 不放 — `asyncio.to_thread` 救不了**:1.5GB 檔在 to_thread 下實測 event loop 凍 6.35s。**sleep-mock 單元測試測不到這件事**(`time.sleep` 會釋放 GIL → 假綠),必須 real-env 探針或真檔實驗。解法 = chunked JSONL(meta 行 + 每 100k rows 一行,逐 chunk loads/dumps),stall 上界縮到單 chunk ~100-150ms。`market_breadth._write_prices_cache/_read_prices_cache` 是樣板。Trigger:任何 >100MB JSON cache 要進 async 服務時。
- **`asyncio.sleep(0)` 不會讓等待中的 timer/IO callback 先跑**:sleep(0) 把本 task 重排 ready queue 前面,剛到期的 callback 排後面 — 插 yield 打散長 sync 段要用 `sleep(0.005)` 之類真掛起(實測 sleep(0) max gap 1898ms vs sleep(0.005) 888ms)。Trigger:在 async orchestrator 內連續跑多段純 Python 重計算時。
- **新 EOD compute 一律掛進 `_fetch_eod_results`,不要 inline await 進 `_do_fetch_market_snapshot`**:掛對位置才拿到 (end_date, universe digest) result cache + shared prices 注入 + refresh 隔離(refresh=true 只 bust intraday,不進 EOD — user 心智模型「重新整理 = 看最新盤中」)。inline 會讓 warm request 重付 parse + refresh 重付 128 次 FinMind。Trigger:market-monitor P5+ 加新 EOD 指標時。

### Market page V2 frontend 慣例(2026-07-02 /feat market-page-v2-frontend 沉澱)

- **App 下的 mode page root 用 `flex-1 min-h-0`,不用 `h-full`**:App root 是 `flex flex-col`(mode nav `shrink-0` + page),flex item 的 `h-full` = 100% **容器**高(900),不是「扣掉 nav 的剩餘空間」(861)→ 頁面下溢 nav 高度被 App `overflow-hidden` 靜默裁切(舊 MarketPage 潛伏此問題,leaderboard 尾列被裁沒人發現)。e2e 量測 viewport 幾何一律 `test.use({ viewport })` 在導航前固定 — `setViewportSize` 後立即量測會撞 resize relayout race(實測假綠:量到舊 720p 幾何)。Trigger:加新 mode 頁 / 寫任何 e2e viewport 量測 spec。
- **`useContainerSize` 的 ref 必掛「恆存 wrapper」**(loading / unavailable / data 三態都 mount 的元素):hook 的 effect deps `[ref, measure]` 皆穩定、null ref 時 early-return 且**永不重跑**,ref 若只掛在 data 分支,冷載入(先 skeleton 後資料)會永遠 0×0 空白。**jsdom 測不到**(getBoundingClientRect 恆 0 + 測試通常 mock 掉整個 hook)— regression lock 用真 hook + polyfill ResizeObserver + stub `Element.prototype.getBoundingClientRect`,rerender loading→data 後 assert svg width(`MarketColdLoad.test.tsx` 是樣板)。Trigger:新元件用 useContainerSize 且有多態渲染(skeleton / 降級)時。
- **devtools MCP 截圖 close-up 用 PIL crop 整頁截圖,不用 `body.style.zoom`**:zoom 會污染 useContainerSize 量測(ResizeObserver 以 zoom 後幾何重排,拍完 reset 也可能留下爆版 layout),實測拍出全黑 / 單 tile 撐滿的廢圖。Trigger:Phase 6 真實環境要 panel 級 close-up 證據時。

### 響應式 / 字級縮放慣例(2026-07-03 responsive 沉澱)

- **全站字級縮放機制 = root font-size media query(≥1920 112.5% / ≥2560 125%)+ 全 rem**:新 code 禁用 `text-[Npx]` px-literal(不吃縮放),SVG 內 fontSize 一律 rem 字串(viewBox 1:1 直接生效);SVG 大標籤用 `chip-theme.ts::svgLabelFont(width)` / `svgLegendFont(width)`(<500px 容器自動降級,手機不疊字)。幾何驅動的動態字級(chip-price-bar rowH 那顆)保留 px。Trigger:新增任何含文字的元件 / SVG renderer。
- **Container query 若門檻邏輯上是「px 版面塞不塞得下」,用 px 任意值不用 rem 級距**:ChipBrokersPanel 曾用 `@md`(28rem),2560 螢幕 root 放大後門檻變 560px > 預設面板寬 420px,桌面反而藏欄。改 `@[400px]:`。Trigger:寫任何 container query 減欄 / 降級。
- **jsdom 沒有 `window.matchMedia`(不是回 matches:false,是 undefined)**:`hooks/useMediaQuery.ts` 已 feature-detect 回 false;判斷方向一律 `(max-width: 1023px)` 判 mobile、桌面為預設分支,vitest 下元件自動走桌面分支。新響應式 JS 分支跟這個 pattern,雙分支共用的 JSX 抽變數不複製。Trigger:元件需要 JS 換容器(非純 CSS 降級)時。
- **延遲 mount 的容器(bottom sheet / modal)內用 useContainerSize,ref + hook 必須宣告在「隨容器 mount 的元件」內部**:掛 parent 的 ref 會踩 null-ref 永不重跑陷阱(§9 market-page-v2 條目的變體)。ChipBubbleView 的 DetailPanel 是樣板。Trigger:sheet / dialog 內放需量測的 SVG 圖表。
- **觸控目標用 Tailwind `pointer-coarse:` variant(4.1+ 內建)加 min-h-11 / py 放大**,桌面視覺零影響;K 線 crosshair 這類 hover 互動在觸控上靠 tap 的 synthetic mousemove 免改即可用(overlay 是 onMouseMove + onClick 才成立,改用 pointer event + pointerType 過濾就會破)。Trigger:新增可互動元件 / 改 chart overlay 事件模型。
### API cancel 傳導慣例(2026-07-03 /perf api-cancel-and-rate-boost 沉澱,三輪修正)

- **前端 fetch 一律接 `AbortSignal`**:`lib/api.ts::get(path, params, options?: { signal? })` 為 base。所有 `api.*` / `optionsApi.*` / `fetchMarketSnapshot` 尾端加 `options?: RequestOptions`;每個 `useQuery({ queryFn: ({ signal }) => api.xxx(..., { signal }) })` 直傳 TanStack Query 內建 controller。**useMutation 沒內建 signal** — 走 `useRef<AbortController | null>` + `useEffect(deps)` cleanup pattern(`useBrokerHistory` 是樣板)。**禁止再回頭寫「裸 fetch(url)」**。Trigger:寫任何新 hook / api method / 新前端 fetch call 時。
- **Cancel 鏈是四環,每環都可能斷,驗收必須驗到最後一環**:browser abort → ①vite proxy destroy upstream(`vite.config.ts` proxy `configure` 掛 `res.on("close")` + `proxyReq.destroy()`,**vite 預設不轉發 abort**)→ ②uvicorn http.disconnect → ③`utils/cancel.py::run_with_disconnect` watcher cancel route task → ④`_run_once` subscriber refcount 歸 0 才 cancel 底層 fan-out(`asyncio.shield` 保護 shared dedup)。2026-07-03 教訓:chrome-devtools 看到 `net::ERR_ABORTED` **只證明第一環**,當時 ①③④ 全斷但表面看起來「修好了」。驗收唯一可信法:FinMind `user_info.user_count` side-channel(打 uncached symbol fan-out → abort → 看 count 停哪;注意 counter 有 5-8s 批次延遲 + rolling window aging 噪音,要先量 idle drift)。Trigger:改動 cancel 鏈任一環 / 懷疑 cancel 沒生效時。
- **Cancel 鏈在 prd 有第五環:Vercel 外部 rewrite ~30s 超時**(拓撲:Vercel 前端 rewrite `/api` → Railway FastAPI,見 `frontend/vercel.json`)。router 超時斷線 = client 斷線,觸發整條 cancel 鏈 — 任何 >30s 的 route 在 prd 必死(`ROUTER_EXTERNAL_TARGET_ERROR`),長計算進度被取消全丟。**長計算(EOD 類)必須與 request 生命週期脫鉤**:module 持有引用的背景 task + inline 小預算 + `xxx_pending` payload 旗標,`finmind_realtime._ensure_eod_task` 是樣板。Railway 磁碟 ephemeral,每次 redeploy cache 全清 = 必冷。Trigger:新增任何可能 >30s 的 endpoint / 排查 prd 502。
- **共用 inflight dedup 必須 shield + subscriber refcount**:`await _inflight[key]`(無 shield)時,asyncio 會把 awaiting task 的取消直接傳進共用 task,一個斷線請求毒殺所有共乘請求(收 CancelledError → 裸 500)。`finmind.py::_run_once` 與 `finmind_realtime._run_once`(2026-07-03 修)是 refcount 樣板;route 邊界另補 CancelledError → 503(還連著才轉,已斷線 re-raise)。Trigger:寫任何新的 inflight dedup / 看到 prd CancelledError 500。
- **FinMind 真瓶頸 = 每小時 6000 requests(rolling window),不是 per-second rate**:一檔冷 `history/major`(days=540)~360 req → **每小時只能冷載入 ~16 檔**;燒乾後全面 402 → 前端 502(HTTPStatusError)/503(JSONDecodeError 是 ValueError 子類)。rate 預設 8/s:拉高只會燒更快 + abort 前已燒的更多。**檢查配額**:`GET api.web.finmindtrade.com/v2/user_info`(Bearer)看 `user_count / api_request_limit`。結構性解法是砍每檔 request 數,不是調 rate。Trigger:出現成串 502/503 / 設計新 fan-out endpoint / 評估冷載入成本時。
- **uvicorn --reload 的 file watcher 在本機會靜默失效**:2026-07-03 實測 worker 子行程 3 小時沒 respawn,期間所有 backend 改動都沒生效(在舊 code 上驗證 = 白驗)。**驗證 backend 改動生效前,先 `Get-CimInstance Win32_Process` 對照 python 子行程 creationDate vs 改動時間**;不符就手動重啟。§9 舊條目「--reload dev server 直接拿來驗證」在 watcher 失效時不成立。Trigger:Phase 5/6 real-env 驗證 backend 改動前。
