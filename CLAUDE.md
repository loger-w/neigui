...
- **共用 cache_key 含日期 ⟹ 每日首請求必冷**:`breadth_prices_<start>_<end>` 的 end = 當日 → 日期翻頁後 key 變 → 當日第一個 snapshot request 付冷啟動(real-env 實測 277.8s),warm 後 ~36.8s(P3 基準 ~30s + P4 第三次 extract pass ≤6s accepted)。**別把當日首請求的 4-5 分鐘當 regression 修**。Trigger:real-env 驗證 snapshot 端點覺得「怎麼突然變慢」時。
