...
### API cancel 傳導慣例(2026-07-03 /perf api-cancel-and-rate-boost 沉澱)

- **前端 fetch 一律接 `AbortSignal`**:`lib/api.ts::get(path, params, options?: { signal? })` 為 base。所有 `api.*` / `optionsApi.*` / `fetchMarketSnapshot` 尾端加 `options?: RequestOptions`;每個 `useQuery({ queryFn: ({ signal }) => api.xxx(..., { signal }) })` 直傳 TanStack Query 內建 controller。**useMutation 沒內建 signal** — 走 `useRef<AbortController | null>` + `useEffect(deps)` cleanup pattern(`useBrokerHistory` 是樣板)。切股票 / 切 mode 時舊 request 網路層立即 abort(chrome-devtools 網路面板顯示 `net::ERR_ABORTED`)。**禁止再回頭寫「裸 fetch(url)」**。Root cause 之前:切 mode 之前的 3008 cold `history/major` 24s 全跑完 status=200 佔 rate slot 阻塞新 symbol(FastAPI/Starlette 預設不 propagate client disconnect + `_run_once` 用 `ensure_future` 頂住 task)。Trigger:寫任何新 hook / api method / 新前端 fetch call 時。
- **Rate limiter 30/s = sponsor-friendly 中位值**:5→72s / 10→36s / 15→24s / 30→12s(360-day fan-out cold),env `FINMIND_RATE_LIMIT_PER_SEC=15` 可 dial back。搭 AbortSignal 傳導(切走立即釋放 slot)避免多 client 疊加撞 429。**若 upstream 拋 429 → dial back env 不改 code**。Trigger:新 fan-out endpoint 想預估冷啟動時。
