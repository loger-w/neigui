...
**狀態(2026-07-02)**:①(a8288ae)②(e2db45b)③(0d47091..841b16a,8 commits)
已全部 merge 進 main,只剩 ④。③ 成果:warm snapshot <1s / refresh <1s /
EOD 計算不卡其他 endpoint / payload 新增 top-level `eod_as_of`。
①②③修正後的真實 payload 已存
`docs/specs/market-monitor-v2/evidence/snapshot_full_2026-07-02_post-fixes.json`
(146KB;oscillator +13.97、44 sectors、無幽靈、eod_as_of=2026-07-02)—
④ 的 fixture 取材以此為準,舊 snapshot_full.json 僅供歷史對照。
...
②/bug sector-override-phantom、③/perf snapshot-hot-path 三輪**已於 2026-07-02 全部
merge 進 main**(warm <1s,UX 驗證不再失真)。
...
- docs/specs/market-monitor-v2/evidence/snapshot_full_2026-07-02_post-fixes.json
  (①②③修正後的真實 payload,fixture 直接取材;舊
  .claude/feat/market-sector-amount-share/evidence/snapshot_full.json 是修正前值,
  僅供歷史對照不要取材)
...
★ Payload 契約事實(audit verified + ①②③後校正,直接照抄,不要信 spec §8 example):
0. (③新增)top-level `eod_as_of: string | null` = 四個 EOD compute 實際用的
   max price date,「資料至 YYYY-MM-DD」直接用它,null 時才 fallback
...
4. mcclellan_series 前 ~38 筆 value=null(EMA 暖機)、序列長度 ~127(整個 pad window)
   ...
   ①修正後量級 ±150 內(post-fixes evidence 實值 +13.97),±100 thrust 可觸發
...
9. sector 維度 = TaiwanStockInfo industry_category ~44-45 類(非 spec 的 32 大類;
   ②修掉幽靈後現值 44,集合日變動),含「其他」;heatmap 以 ~45 cells 彈性設計;
   onSectorClick 參數 = 中文 sector 名字串(與既有
...
- 🟢 components/MarketSectorBreadthHeatmap.tsx(~44-45 cells 彈性;onSectorClick)
...
呈現紀律(全部測試 lock):
- 每個新 panel 標「資料至 YYYY-MM-DD」— 日期來源用 payload `eod_as_of`(③已加,
  real-env 驗過;null 時 panel 文案避免「今日」改「最近交易日」);盤中新欄位
  全是 T-1 EOD,header 的「即時」lag pill 不適用這四個 panel
- market-types.ts 補一個 vitest:把 snapshot_full_2026-07-02_post-fixes.json 當 fixture 過型別
