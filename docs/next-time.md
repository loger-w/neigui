...
- **發行商排行 v2:條款分層聚合**:v1 composite 未按價內外/天期分層,real-env 實證元大 positive control 未過(大書商深價外 penny 權證墊高 iv_std/spread 中位數,排行反映產品組合結構多於造市品質 — 臺大論文的分層警告命中)。v2 方向:per-warrant 指標在 (moneyness band × 天期 band) 分層內正規化再聚合(archive 有 s、terms 有 strike,資料齊)。觸發:user 要拿排行做發行商選擇決策時(現版標注「僅供排序參考+跨規模保留」)
