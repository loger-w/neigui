...
- (原「Visual baseline 更新」條目已由 e2e-update-snapshots workflow(PR #47)收割刪除,2026-07-20:equity-2330 收 C4 幾何 + options-top 收 options-page-v2 漂移 + V4-V6 響應式三張首生成,變更圖已人眼比對。**留意:workflow 的 create-pull-request 步驟已被 repo 設定擋下(Actions 不可開 PR),branch 會推上但 PR 要手動開** — 下次跑該 workflow 時直接 `gh pr create --head e2e/refresh-visual-baselines`)
