...
- **`DateField` 無觸控放大**(review Std-5):元件固定 `h-8`,未套 frontend-conventions「可互動元件加 `pointer-coarse:min-h-11`」;個股頁 / 選擇權頁 / 分點反查三處共用同一元件,應在元件層統一處理而非各 caller 補。觸發:手機觸控體驗被反映、或下次動 `date-field.tsx` 時。〔2026-09-25 更新:「動 `date-field.tsx`」已觸發兩次 — mod/date-field-dark-calendar(de009cd,僅 JSDoc)與 fix/date-field-partial-input(可提交防護)— 兩次皆因不在該次 scope 而延後,未處理。觸發條件改為:手機觸控體驗被反映,或以 DateField **外觀 / 尺寸**為主題的下一個 /mod。〕
- **日期草稿防護兩份**(fix/date-field-partial-input review Std-3,2026-09-25):DateField 包裝路徑已內建「可提交」防護,`BrokerFlowsPanel` 仍在純原生路徑上自帶一份(`DATE_RE` / 2000-01-01 下限 / 同名 `isCommittableDate`,另有 300ms debounce 與 render 時現算的今天上限)。收斂 = 分點反查改用 `onValueChange` + `max={today}`,debounce 視需要下沉到 DateField — 屬 🔴/🔵 改動,非順手活。觸發:下次動 BrokerFlowsPanel 日期欄位、或第 4 個日期欄位出現時。
- **逐位打「日」仍多查一輪**(同上 diagnosis Out of scope):打 `15` 時先出現 `…-01`(合法日期)就提交一次,再提交 `…-15`;非錯誤請求,只是多一輪抓取。個股 / 選擇權頁無 debounce(分點反查有 300ms)。觸發:user 反映輸入日期時畫面閃兩次、或配額吃緊時,評估 DateField 內建 debounce。
- **日期欄位「今天」按鈕**(2026-09-25 grilling 決議):fix/date-field-partial-input 後,月曆「清除」= 不動作、失焦還原(user 確認保留,理由見該 diagnosis「Review 追記」);舊的「清除 = 以空日期看最新」不恢復 — 清除與逐段 Backspace 同送空字串無法區分。若要一鍵回到最新,做明確的「今天」按鈕(個股 / 選擇權頁共用,設為 `todayStr()`,非交易日交給後端 `no_trading_day` 退回)。觸發:user 反映翻舊日期後回最新不便時。
...
- (原「頂欄「重新整理」鈕常態換行」條目已於 2026-08-21 收割刪除,user 拍板方案 A:插槽改 `absolute left-0` 浮在 `px-2.5` 的 padding 內、圖示 10px,鈕寬 102 → 78px(82px 在 Windows 375 仍差 0.4px 掉行,preset 組 249.4px 是刀口)。real-env:375 同行(tabs 299 → 257)、1256 不換行餘裕 24px(門檻 ≈1232);券差頁同改。截圖 08 / 09 / 10b。changelog 0.49.2)
