...
- **[bug 候選] iv backfill「weekday empty」疑似 transient 而非真非交易日**:E2 實測 TWSE MI_INDEX 未命中其 server cache 的歷史查詢 10-25s,且慢查詢可能「內部超時回 stat=OK 空表」(2026-07-10 週五 11.3s 回空、2026-07-03 週五 16.1s 回 26,347 rows)— R15 的單次 retry 不足,empty 日每次啟動重掃永遠做白工,且 drift 窗可能真缺交易日資料。修法候選:empty 日寫短 TTL marker 檔、或 retry 拉長間隔重驗。觸發重評估:drift 資料被質疑缺日、或 backfill 掃描時間被抱怨時(屬 bug 調查,勿混 perf)
