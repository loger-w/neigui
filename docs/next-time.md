...
- **「empty == 非交易日」假設的其他複本**:warrants snapshot `_candidate_dates` 回退、warrant_flow `_candidate_dates`、market_breadth per-day 都有「上游空回 → 當非交易日跳過」推斷;它們不落永久檔(每請求重評)故影響輕於 backfill,但 transient 空回同樣可能造成單次錯回退。觸發重評估:快照 as_of 異常回退、或 flow/breadth 缺日被質疑時
