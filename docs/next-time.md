...
- **權證快照冷 build 實測 63s(2330 首請求,全市場 ~36k 檔 IV 反解)**:每日一次可接受,但可考慮 (a) 啟動預熱 task(lifespan kickoff 背景 build)或 (b) IV 反解 thread offload。觸發重評估:user 抱怨早盤首開慢、或加第二個吃快照的功能時(2026-07-15 user 已反映 API 載入偏慢 — /perf warrant-api 排程中,先 profile 再動)
