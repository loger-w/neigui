...
H. **Git 推送紀律**:流程分支 push(PR 收尾)自動;push main / force / merge 前必列清單給 user 確認;自主模式不豁免(v3 起有 push-gate hook 硬攔後盾,見 3.5;2026-07-07 起 merge ask = PR 收尾單一確認點)。
...
| 8 | 收尾 | TDD tag 機驗 script 化(`check_feat_tags.py`,配對 / 豁免規則固化 + 自帶 pytest)+ artifact commit + **branch-lifecycle 收尾節 PR 收尾(push → PR → review 補齊 → merge 確認 → auto-merge)**(顯式覆寫底層 skill 的三選一互動) |
...
**`/auto` 自主模式契約**:退出條件可機械判定;自動敲定的決策標 `[auto-default]` 供 audit;**merge 確認框 = 唯一必停檢查點**(2026-07-07 拍板;流程分支 push / 開 PR 自動)、破壞性操作永遠停。
...
- **branch-lifecycle(v3 新增;2026-07-07 v2:PR 收尾取代 local merge 預設,`docs/specs/harness-pr-lifecycle/design.md`)**:開工節(不在 main 停下問 → fetch 同步 → prefix 對照表開分支)/ 收尾節(Done+全綠 gate → 漂移判準 → review 補齊 → push 分支 + 開 PR → merge 確認框 → `--rebase` auto-merge;離線 fallback local ff)/ 異常表十一列(rebase 衝突、分岔、撞名、不在 main、放棄、merge deny、main 領先等)。**這份設計經自家 design-reviewer agent 兩輪 review 收斂**(§5 story 7)。
...
- **`harness-push-gate.py`(v3)**:`git push` / `gh pr merge` → `permissionDecision: "ask"` 強制跳 user 確認框,無視 session permission mode — 鐵則 H 從純文字變硬攔;matcher 覆蓋 **Bash + PowerShell 雙工具**(2026-07-07 起 block-no-verify / safety-hooks 亦補齊雙工具 + PS 專屬 secrets pattern,繞過面關閉)。fail-closed:hook 內部錯誤仍回 ask。2026-07-07 起:流程分支 push(嚴格 fullmatch)放行,merge ask 升格為 PR 收尾單一確認點。
