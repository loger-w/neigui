...
**`/auto` 自主模式契約**(2026-07-07 由 /goal 改名,避免遮蔽 Claude Code 內建 /goal):退出條件可機械判定;自動敲定的決策標 `[auto-default]` 供 audit;**merge 不必停**(v3 拍板:收尾 gate 全綠即自動,local merge 可逆)、push/PR/破壞性操作永遠停。
...
- **`harness-push-gate.py`(v3)**:`git push` / `gh pr merge` → `permissionDecision: "ask"` 強制跳 user 確認框,無視 session permission mode — 鐵則 H 從純文字變硬攔;matcher 覆蓋 **Bash + PowerShell 雙工具**(2026-07-07 起 block-no-verify / safety-hooks 亦補齊雙工具 + PS 專屬 secrets pattern,繞過面關閉)。fail-closed:hook 內部錯誤仍回 ask。
