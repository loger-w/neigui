...
0. **Phase 0|工作區**:呼叫 `branch-lifecycle` 開工節
...
2. **Phase 2|Profile 找真實 bottleneck**:呼叫 `systematic-debugging`(profile 假說
   ...
   - **退出條件**:**3 輪 profile 仍定位不到** bottleneck → 套鐵則 F 停下回報(可能是分散式慢、
     無單一 bottleneck,該重新定義目標)
...
4. **Phase 4|實作**:
   ...
   - Cache 類策略:實作前 optimize-plan.md 該策略節必列 **invalidation 三欄** — 失效時機 /
     bust 觸發點(誰在哪呼叫)/ 驗證測試名;缺任一欄不准實作(cache 是最容易引入 bug 的優化)
...
## 失敗 routing
- **沒達標** → 不是「優化成功只是不夠多」,hypothesis 可能錯,回 Phase 2 重新 profile
  (瓶頸可能已移到別處)
...
- ❌ 沒 profile 靠猜哪裡慢
- ❌ optimize-plan.md 沒列 invalidation 三欄就加 cache
- ❌ 為了 1% 改善加 100% 複雜度
- ❌ 改完沒重新量測就說「應該更快了」
