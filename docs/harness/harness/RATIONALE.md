...
2026-07-27 review 補修六處(2 P1 + 4 P2):auto.md 替代條件段補分流例外句(原「無文件但無方向性抉擇 → 推進」與 feat.md「拍板必停」在口頭已成形方案 + 純實作級決策 case 正面矛盾 — 兩檔兩說,P1);refs/feat-phase0-2.md 寫入要求補「分流判定記錄」條(gate 原無稽核痕跡,/auto 下判模糊可續跑、判已成形必停的誘因梯度單向指向不觸發,P1);auto.md 建議表 /feat S 級補停等註(✓ 與必停拍板的張力,P2);load-manifest.json feat-L Phase 0 補 grilling 條件條目(分流觸發時載入帳漏算,P2);判準節補「條件 1 單獨成立仍縮提案」句(全成形無決策點 case 原落回 2-3 方案儀式,P2);/mod 側接入記 next-time.md /mod 改版清單(mod.md Phase 2 聚焦只縮主題不換姿態,brainstorming 2-3 方案 checklist 仍強制,痛點同在 — P2 不本批修)。
2026-07-27 復審二輪補修(1 P1 + 2 P2 + 1 P3):feat.md 分流句補「僅條件 1 成立」例外半句(前輪縮減句與 feat.md「照現行 brainstorming 不變」兩檔兩說 — fix-induced,同型於前輪 auto.md P1,修法標準一致,P1);第三類(條件 1 中、條件 2 未中)/auto 行為 user 拍板 — 方案完整且無 counter-proposal → 推進標 [auto-default],有 counter-proposal / 待討論點 → 停等拍板、確認步不得自問自答,落 refs 判準節 + auto.md 例外句尾(P2);auto.md「仍必停」清單本體 +1 項 grilling 拍板(feat.md「視同清單」指涉原落空、/goal 優先序句不背書例外句,P2);交接檔「五個真實檔」計數改正(同檔還原路徑自書四檔,P3)。還原 = 各處刪「2026-07-27 user 拍板 / review 補修」句 + 清單第 4 項。
...
2026-07-27 user 逐條拍板收件匣 5 條 + 小尾巴全修。(A1)`harness_load_estimate.py` 加 baseline 防呆:`--before` profile 每筆記錄須帶 `baseline_bytes`(`--write-baseline` 在改版前凍結),缺欄或與實體檔不符 → exit 2 拒絕輸出降幅(事故:改版後跑 baseline 得 -23.17%,真值 +4.4%);既有 `*-before` profile 不補凍(其檔案已被改寫,凍現值 = 凍謊言,要重建回 git 歷史取改版前檔案)。(A3)加 `--peak`:peak = 常駐(無 phase 欄)+ 最重 phase,降幅模式對兩側峰值取比 — 下輪瘦身 SC 用峰值降幅。hooks 測試 130 → 137。(A4)chore.md 步驟 1 加「Harness 攢批」強制句(cache 失效 9.5× 實測,原僅本檔知識)。(A5)refs 分層合併 rejected 結案 — 可無痛合併僅 feat 專屬幾支,收益不足。(A2)CLAUDE.md 瘦身輪 scheduled:排四 command 同步批之後,列管 next-time.md。(C1)dispositions.json 6 條過期 rows 更新為 07-26 改版後繼詞彙(Finding 處置分級 / Review 輪數 / rollbacks / P2 彙總計數 / review-protocol.md / per_file 廢除宣告)。(C2)mirror 納 6 支改寫複製 skill(各目錄 *.md,19 檔;grilling / grill-me 原文照抄件刻意不納)。(C3)writing-plans L158 殘句補回 `superpowers:` 前綴(未複製之條件式罕用項慣例)。(A2 執行,同日四 command 同步批後)專案 CLAUDE.md 瘦身輪:§2 Python 風格整節移出成專案 skill `backend-conventions`(新)、§3 React/TS 風格 + §4 分點名稱顯示細節併入 `frontend-conventions`(§4 留契約核心句)、§1 E2E 判準段與 .env 段壓縮;14,015 → 10,354 bytes(−26.1%,量法 stat().st_size 前後對照);§8 索引補 backend-conventions row、沉澱目的地行改「跨檔契約 → §4;風格 → conventions skill」。判準 =「只留每 session 都用的」(拍板於收件匣 A2)。還原 = 兩 skill 內容搬回 + revert 該 commit。還原:腳本 revert 該 commit;chore.md 刪攢批句;mirror DIR_MAPS / ORPHAN_SCOPES 刪 6 條目 + 刪鏡像 skills/<6支>/ 目錄;dispositions rows 見 git 歷史。

**[2026-07-27 封批復審:無 P0/P1,3 P2 即修]**
限縮式對抗復審(入口 `neigui/docs/harness/REVIEW-2026-07-27-close.md`)審 15e14f8 / 43a16d2 / 357301a。全數驗過無失:§2/§3/分點名稱逐字搬移零滅失、dispositions 6 rows 新 check 字串逐條鎖住原機制(rollbacks ↔ feat.md meta-cycle 條款)、mod 與 feat-phase0-2 的 `[amendment]` 格式一致、退出軸三處自洽、A2 通則與 design-reviewer 無兩檔兩說(design.md 確有 changelog 義務)。3 P2 同日修:(1) 專案 CLAUDE.md `.env` 指路行對 `FRONTEND_ORIGIN` 空指(不在 finmind-conventions)→ 改指 `backend/.env.example`;(2) 專案 skill market-pipeline 懸空 §3 引用 → 改指 frontend-conventions(357301a「無懸空引用」grep 漏了專案側 `.claude/skills/`);(3) change-spec-reviewer / refactor-plan-reviewer 輸入段殘留「changelog 摘要」→「修復摘要」(兩文件無 changelog,43a16d2 改 A2 用詞時漏帶 agent 檔)。同日 user 指示對其餘 command 檔(feat / bug / perf / auto / chore)做同型交叉面檢查:引用面全實(feat.md 8 支 refs + auto-wave 皆存在、auto.md 分流句雙向可達、bug/perf/chore 無過期詞彙),僅多揪 1 條同類殘尾 — reviewer-preamble.md Cross-round 節「changelog 摘要」→「修復摘要」(四支 reviewer 共用檔,同 43a16d2 漏帶;design-reviewer 自檔的「changelog 摘要」正確保留)。還原:各處刪句還原,mirror 重跑 `--fix`。
